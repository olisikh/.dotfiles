...
function dimHintLabel(
	theme: PiForegroundTheme | null | undefined,
	text: string,
): string {
	try {
	...
	pi.on(
		...
		async (_event: SessionStartEvent, ctx: ExtensionContext) => {
			...
			ctx.ui.setEditorComponent(
				(tui: TUI, editorTheme: EditorTheme, kb: KeybindingsManager) => {
					...
					editor.render = (width: number) => {
						...
						if (lines.length > 0) {
							...
							lines[last] = stripModeLabel(lines[last], width, editor.borderColor);
						}
