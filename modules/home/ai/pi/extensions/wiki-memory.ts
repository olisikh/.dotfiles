...
async function finalize(input: FinalizeInput, sessionID: string, directory: string): Promise<string> {
	...
	const metadata = [
		...
		`kind: ${noteKind}`, `domains: [${yamlScalar(domain)}]`, `tags: [${yamlScalar(domain)}, "pi", "wiki-memory", ${yamlScalar(input.kind)}]`,
	...
	await appendFile(path.join(root, "90 System", "logs", "pi-wiki-memory.md"), `\n## ${today()} — ${input.kind}: ${title}\n- id: ${id}\n- domain: ${domain}\n`);
	await updateRegistry(root, id, { path: path.relative(root, target), title, kind: noteKind, domains: [domain], tags: [domain, "pi", "wiki-memory", input.kind], status: "active", aliases: [], source_refs: [], updated: today() });
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "finalize_wiki",
		label: "Finalize Wiki",
		description: "Persist a redacted synthesis after an approved plan, validated implementation, or durable decision.",
		parameters: Type.Object({
			kind: Type.Union([Type.Literal("plan"), Type.Literal("implementation"), Type.Literal("decision")]),
			title: Type.String(), topic: Type.Optional(Type.String()), summary: Type.String(),
			decisions: Type.Optional(Type.Array(Type.String())), evidence: Type.Optional(Type.Array(Type.String())),
