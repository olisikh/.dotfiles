...
type RuntimePermissionManager = { isYoloEnabled: () => boolean };
...
const PERMISSION_SERVICES_KEY = PI_RUNTIME_SYMBOLS.permissionServices;
...
function getYoloState(): YoloState {
...
function permissionService(): RuntimePermissionService | undefined {
  ...
  // v30 scopes permission services by Pi session; never bind another node's.
  const services = Object.getOwnPropertyDescriptor(
    globalThis,
    PERMISSION_SERVICES_KEY,
  )?.value;
  if (!(services instanceof Map)) return undefined;

  const service = services.get(activeSessionId);
  return isRecord(service) ? (service as RuntimePermissionService) : undefined;
}
...
async function handleYolo(
  ...
  if (error) {
    ctx.ui.notify(
      `Unable to change YOLO mode: ${error}`,
      PI_THEME_COLORS.error,
    );
    return;
  }
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("session_start", (_event: SessionStartEvent, ctx: ExtensionContext) => {
    ...
    getYoloState().enabled = restoreYoloState(ctx);
  ...
  pi.registerCommand("yolo", {
    description:
      "Toggle permission-system YOLO mode for the current Pi session",
