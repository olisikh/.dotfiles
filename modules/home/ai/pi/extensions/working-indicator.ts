...
function textLength(
	content: string | AssistantMessage["content"] | undefined,
): number {
	if (content === undefined) return 0;
	if (typeof content === "string") return content.length;

	return content.reduce(
		(length: number, block: { type: string; text?: string }) =>
...
export default function (pi: ExtensionAPI) {
	...
	const handleMessageUpdate = (
		...
		switch (assistantEvent.type) {
			...
			case "toolcall_start":
				...
				break;
			case "done": {
				const finalTextLength = textLength(assistantEvent.message.content);
	...
	const handleTurnEnd = (event: TurnEndEvent, ctx: ExtensionContext) => {
		activeContext = ctx;
	...
	pi.on(
		...
		(_event: ToolExecutionStartEvent, ctx: ExtensionContext) =>
			handleToolStart(ctx),
	);
	pi.on(
		"tool_execution_end",
		(_event: ToolExecutionEndEvent, ctx: ExtensionContext) => handleToolEnd(ctx),
	);
