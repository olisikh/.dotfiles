#! /nix/store/546i2hymi2imlm7xwrjnmm28cvqk09jn-python3-3.13.5-env/bin/python3.13
import functools
import grp
import hashlib
import json
...
def download_tarball(
    ...
    if pkg["name"] == "libloading":
        source_path = Path(__file__).parent / filename
        if not source_path.exists():
            eprint(f"Expected '{filename}' in script directory, but it was not found.")
            return

        shutil.copy2(source_path, tarball_out_dir)
        eprint(f"Copied '{filename}' to '{tarball_out_dir}'.")

        try:
            uid = pwd.getpwnam("_nixbld1").pw_uid
            gid = grp.getgrnam("nixbld").gr_gid
            os.chown(tarball_out_dir, uid, gid)
            eprint(f"Changed ownership of '{tarball_out_dir}' to _nixbld1:nixbld.")
        except KeyError as e:
            eprint(f"User or group not found: {e}")
        except PermissionError:
            eprint("Permission denied while changing ownership. Try running as root.")
        return

    url = get_download_url_for_tarball(pkg)
    expected_checksum = pkg["checksum"]

    eprint(f"Fetching {url} -> {tarball_out_dir}", end="")

    result = download_file_with_checksum(session, url, tarball_out_dir)
    calculated_checksum = result["checksum"]
...
#     if pkg["name"] == "libloading":
#         eprint(
#             f"Skipping {pkg["name"]} download, place the file into {tarball_out_dir} yourself"
#         )
#         return
#
#     url = get_download_url_for_tarball(pkg)
